...
const Countries = () => {
  ...
  const handleToggleFavorite = async (e, countryCode) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8070/api/v1/auth/favorites${isFavorited ? `/${countryCode}` : ''}`,
        {
  ...
  const fetchUserFavorites = async () => {
    try {
      const res = await fetch('http://localhost:8070/api/v1/auth/getfavorites', {
        method: 'GET',
        credentials: 'include',
      });
      const data = await res.json();
  ...
  return (
    <Layout>
      <section className="py-5 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-2xl md:text-3xl lg:text-4xl sm:text-4xl font-bold text-teal-700 text-center mb-6">
            Country Flags
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-start gap-2 mb-10">
            <input
              ...
              className="flex-1 min-w-[150px] px-4 py-2 border border-teal-300 rounded-md bg-teal-50 hover:bg-teal-100 shadow-sm focus:outline-none focus:ring-2 focus:ring-teal-400 z-10"
            />
            <FilterMenu
            ...
            <button
              ...
              className={`flex items-center gap-1 px-1.5 py-1.5 rounded-md shadow-sm border transition bg-teal-600 hover:bg-teal-700 z-13
          ...
          {/* Country Cards */}
          {loading ? (
            <div className="text-center text-teal-600 text-lg">Loading countries...</div>
          ) : countries.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 z-10">
              {(showFavoritesOnly
                ...
                <div
                  key={index}
                  className="relative bg-gradient-to-r from-teal-50 to-teal-100 rounded-lg shadow-sm hover:shadow-md p-4 text-center transition z-10"
                  ...
                  <button
                    className={`absolute top-2 right-2 p-2 rounded-full transition 
                      ${favoriteCodes.includes(country.cca3) 
                        ? 'bg-white/60 hover:bg-amber-100/60' 
                        : 'bg-white/60 hover:bg-amber-100/60'
                      }`}
